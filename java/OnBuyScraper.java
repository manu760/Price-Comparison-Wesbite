...
/**
 * This class is to scrape the data from onbuy website.This class extends the
 * webScraper class having  threads.
 */
public class OnBuyScraper extends WebScraper {
    ...
    public void run() {
        ...
        driver.get("https://www.jdwilliams.co.uk/shop/c/electricals/computing/laptops/f/Brand--Apple");


        List<WebElement> products = driver.findElements(By.className("product__item"));


        for (int j = 0; j < products.size(); j++) {
            try {
                WebElement items = products.get(j);


                Product product = new Product();

                String title = items.findElement(By.className("product__title")).getText();
                String price = items.findElement(By.className("product-price__now")).getText();
                String productUrl = items.findElement(By.className("product__link")).getAttribute("href");

                product.setName(title);
                product.setPrice(price);
                product.setUrl(productUrl);
                product.setStore("JdWilliams");



                Laptop laptop = new Laptop();

                String brand = getBrand(items.findElement(By.className("product__title")).getText());

                String model = getModel(items.findElement(By.className("product__title")).getText());
                String color = getColor(items.findElement(By.className("product__title")).getText());
                String storage = getStorage(items.findElement(By.className("product__title")).getText());
                String displaySize = getDisplay_size(items.findElement(By.className("product__title")).getText());
                String urlImage = items.findElement(By.className("js-plp-image")).getAttribute("src");
                String ram = getRam(items.findElement(By.className("product__title")).getText());
        ...
        driver.get("https://www.jdwilliams.co.uk/shop/c/electricals/computing/laptops/windows-laptops/f/Brand--Dell");


         products = driver.findElements(By.className("product__item"));
        ...
        for (int j = 0; j < products.size(); j++) {
            try {
                ...
                product.setPrice(price);
                product.setUrl(productUrl);
                product.setStore("JdWilliams dell");
