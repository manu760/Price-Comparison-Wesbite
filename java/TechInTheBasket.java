...
/**
 * This class is to scrape the data from techInTheBasket website.This class extends the
 * webScraper class having different threads.
 */
public class TechInTheBasket extends WebScraper {
    ...
    public void run() {
        ...
        driver.get("https://www.techinthebasket.com/computing/laptops/apple.html");
            ...
            try {
                ...
                if (staleElement == true) {
                    try {
                        ...
                        String title = items.findElement(By.className("product-item-link")).getText();
                        String price = items.findElement(By.className("price")).getText();
                        String productUrl = items.findElement(By.className("product-item-link")).getAttribute("href");

                        product.setName(title);
                        product.setPrice(price);
                        product.setUrl(productUrl);
                        product.setStore("TechInTheBasket");


                        Laptop laptop = new Laptop();

                        String brand = getBrand(items.findElement(By.className("product-item-link")).getText());

                        String model = getModel(items.findElement(By.className("product-item-link")).getText());
                        String color = getColor(items.findElement(By.className("product-item-link")).getText());
                        String storage = getStorage(items.findElement(By.className("product-item-link")).getText());
                        String displaySize = getDisplay_size(items.findElement(By.className("product-item-link")).getText());
                        String urlImage = items.findElement(By.className("product-image-photo")).getAttribute("src");
                        String ram = getRam(items.findElement(By.className("product-item-link")).getText());
